...
// Simulate the exact JS string that comes out of parsing the JSON container logged
const parsedString = JSON.parse('"' + rawJsonStr.replace(/"/g, '\\"') + '"');

console.log("Original parsing of decoded string...");
try {
  JSON.parse(parsedString);
  console.log("Parsed perfectly without cleanup!");
} catch (e) {
  console.log("Original parse failed:", e.message);
  runCleanup(parsedString);
}
