...
const LiveView: React.FC<LiveViewProps> = ({ 
  ...
  // Posição "commitada" (só muda ao soltar o arraste). Durante o arraste, o
  // movimento é aplicado direto no DOM via dragRef; o render usa este state,
  // evitando ler positionRef.current durante a renderização.
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (isDragging.current) {
        ...
        // Commita a posição final no state para os próximos renders.
    ...
    return () => {
      ...
      // Reset apenas do estado global (userSelect no body). O estilo do próprio
      // dragRef não precisa ser limpo: a limpeza só roda na desmontagem, quando o
      // elemento já está sendo removido do DOM.
      document.body.style.userSelect = '';
    };
  }, []);
            ...
            {visionType && videoStream ? (
              <>
                ...
                <video
                  ...
                  playsInline
                  ref={(el) => {
                    // Só reatribui se mudou, senão o vídeo recarrega e pisca a cada re-render.
        ...
        <div className="p-3 pt-0 flex flex-col gap-3">
          ...
          <div className="flex justify-between items-center bg-white/5 p-1 rounded-xl border border-white/5">
            <div className="flex gap-0.5">
              <button 
                onClick={onToggleMic}
                className={`p-2 rounded-lg transition ${isMicEnabled ? 'bg-(--accent) text-white shadow-md' : 'text-(--text-secondary) hover:bg-white/5 hover:text-white'}`}
              ...
              <button 
                ...
                className={`p-2 rounded-lg transition ${visionType === 'camera' ? 'bg-green-600/90 text-white shadow-md' : 'text-(--text-secondary) hover:bg-white/5 hover:text-white'}`}
                title="Câmera"
              >
              ...
              <button 
                ...
                className={`p-2 rounded-lg transition ${visionType === 'screen' ? 'bg-green-600/90 text-white shadow-md' : 'text-(--text-secondary) hover:bg-white/5 hover:text-white'}`}
              ...
              <button 
                ...
                className={`p-2 rounded-lg transition ${isProactiveEnabled ? 'bg-(--accent) text-white shadow-md' : 'text-(--text-secondary) hover:bg-white/5 hover:text-white'}`}
                title="Proatividade"
              >
            ...
            <div className="relative model-menu-container">
              <button 
                ...
                className={`px-2 py-1 text-[10px] font-semibold rounded-lg border border-white/5 bg-white/5 hover:bg-white/10 text-(--text-secondary) hover:text-white transition flex items-center gap-1`}
                title="Modelo LIVE"
              >
              ...
              {showModelMenu && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 bg-(--bg-sidebar-solid) border border-(--border-light) rounded-xl p-1 min-w-[150px] shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  {LIVE_MODEL_OPTIONS.map(opt => (
                    <button 
                      ...
                      className={`w-full text-left px-2 py-1.5 rounded-lg transition ${liveModel === opt.id ? 'bg-(--accent-bg) text-(--accent-text) font-bold' : 'hover:bg-white/5 text-(--text-secondary) hover:text-white'}`}
                    >
                      <div className="text-[10px] font-semibold text-(--text-primary)">{opt.name === 'Gemini 2.5 Flash Live' ? '2.5 Live' : '3 Live'}</div>
                      <div className="text-[8px] text-(--text-placeholder) mt-0.5 leading-tight">{opt.desc}</div>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="relative">
              <button 
                ...
                className={`p-2 rounded-lg transition ${showVoiceMenu ? 'bg-(--accent) text-white' : 'text-(--text-secondary) hover:bg-white/5 hover:text-white'}`}
                title="Voz"
              >
          ...
          <div className="relative flex items-center">
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-3 pr-10 text-[11px] text-(--text-primary) outline-none focus:border-(--accent-border) transition"
            />
            <button 
              onClick={handleSendText}
              disabled={!inputText.trim()}
              className={`absolute right-1.5 p-1.5 rounded-lg transition ${inputText.trim() ? 'bg-(--accent) text-white hover:scale-105' : 'text-(--text-placeholder) cursor-not-allowed'}`}
            >
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4 relative overflow-hidden animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="absolute top-6 right-6 z-20 flex gap-2">
        <div className="relative">
          <button 
            onClick={handleToggleVoiceMenu}
            className={`p-3 rounded-2xl transition shadow-xl border ${showVoiceMenu ? 'bg-(--accent) border-(--accent-border) text-white' : 'bg-(--bg-sidebar) border-(--border-light) text-(--text-secondary) hover:text-white'}`}
          >
        ...
        <button 
          onClick={onToggleMic}
          className={`p-3 rounded-2xl transition shadow-xl border ${isMicEnabled ? 'bg-(--accent) border-(--accent-border) text-white shadow-(--accent-glow)' : 'bg-(--bg-sidebar) border-(--border-light) text-(--text-secondary) hover:text-white'}`}
          title={isMicEnabled ? "Desativar Microfone" : "Ativar Microfone"}
        >
        ...
        <button 
          onClick={onToggleCamera}
          className={`p-3 rounded-2xl transition shadow-xl border ${visionType === 'camera' ? 'bg-green-600 border-green-400 text-white shadow-green-500/20' : 'bg-(--bg-sidebar) border-(--border-light) text-(--text-secondary) hover:text-white'}`}
          title="Ativar Câmera"
        >
        ...
        <button 
          onClick={onToggleScreen}
          className={`p-3 rounded-2xl transition shadow-xl border ${visionType === 'screen' ? 'bg-green-600 border-green-400 text-white shadow-green-500/20' : 'bg-(--bg-sidebar) border-(--border-light) text-(--text-secondary) hover:text-white'}`}
          title="Compartilhar Tela"
        >
        ...
        <button 
          onClick={onToggleProactive}
          className={`p-3 rounded-2xl transition shadow-xl border ${isProactiveEnabled ? 'bg-(--accent) border-(--accent-border) text-white shadow-(--accent-glow)' : 'bg-(--bg-sidebar) border-(--border-light) text-(--text-secondary) hover:text-white'}`}
        ...
        <button 
          onClick={onClose}
          className="p-3 bg-(--bg-sidebar) border border-(--border-light) text-(--text-secondary) hover:text-red-400 hover:border-red-500/30 rounded-2xl transition shadow-xl"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-4xl relative z-10">
        {status === 'connecting' ? (
          <div className="flex flex-col items-center gap-8 w-full max-w-sm">
            <div className="w-24 h-24 rounded-full bg-(--accent-bg) flex items-center justify-center text-(--accent-text) animate-pulse relative">
                <div className="absolute inset-0 rounded-full border-4 border-(--accent-border) animate-spin" style={{ borderTopColor: 'var(--accent-text)' }}></div>
                <Volume2 className="w-8 h-8" />
            </div>
            <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5 relative">
                <div className="absolute inset-y-0 bg-(--accent) animate-loading-bar shadow-[0_0_15px_var(--accent-glow)]"></div>
            </div>
