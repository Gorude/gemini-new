  ...
  return (
    ...
    >
      <div ref={containerRef} className="flex-1 min-w-0 relative bg-[var(--bg-main)]/50 overflow-hidden">
        ...
        {/* Legend */}
        <div 
          className={`absolute left-6 p-3 bg-[var(--bg-sidebar)]/80 backdrop-blur-md rounded-2xl border border-[var(--border-light)] text-[11px] space-y-3 pointer-events-auto z-50 select-none shadow-2xl transition-all duration-300 ${showControls ? 'bottom-[304px] sm:bottom-6' : 'bottom-6'}`}
      ...
      {showControls && (
        <div className="absolute sm:relative left-0 sm:left-auto right-0 bottom-0 sm:top-0 h-[280px] sm:h-auto w-full sm:w-80 flex-shrink-0 bg-[var(--bg-sidebar)]/95 backdrop-blur-3xl border-t sm:border-t-0 sm:border-l border-[var(--border-light)] p-3 sm:p-4 flex flex-col gap-4 sm:gap-6 z-[60] overflow-y-auto custom-scrollbar shadow-2xl animate-in slide-in-from-bottom-4 sm:slide-in-from-right-4 duration-300">
          ...
          <div className="mt-auto p-3 bg-[var(--bg-chat-hover)] rounded-2xl border border-[var(--border-light)] space-y-2 max-sm:hidden">
